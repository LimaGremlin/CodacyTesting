...
import com.wayapay.ng.landingpage.databinding.FragmentCreateGroupImageBinding
 ...
 * Use the [CreateGroupImageFragment.newInstance] factory method to
 * create an instance of this fragment.
 */
class CreateGroupImageFragment : Fragment() {

    private lateinit var binding: FragmentCreateGroupImageBinding
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(layoutInflater, R.layout.fragment_create_group_image, container, false)
        ...
        nextButton.setOnClickListener {
            val group = viewModel.getWayaGroup().value ?: WayaPages()

            val returnIntent = Intent()
            returnIntent.putExtra(IntentBundles.GROUP_KEY, group)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == RequestCodes.CROP_IMAGE && resultCode == Activity.RESULT_OK){
            ...
            if(whichImage == 1) {
                group.profileImageUri = uri.toString()
                group.profileImagePath = filePath ?: ""
            }
            if(whichImage == 2) {
                group.headerImageUri = uri.toString()
                group.headerImagePath = filePath ?: ""
            }
