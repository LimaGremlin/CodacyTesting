package com.wayapaychat.remote.interceptor.responseinterceptor
        ...
        return when (responseBody) {
            is JSONObject -> {
                newResponseBody = when {
                    responseBody.has("status") && responseBody.get("status").equals(false) -> {
                        /**
                         * An error from the server
                         */
//                        val errorArray = responseBody.getJSONArray("errors")
                        val errorMessage = responseBody.get("message")
                        JSONObject().apply {
